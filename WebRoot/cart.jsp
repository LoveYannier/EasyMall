  ...
  <head>
    ...
    function changeNum2(pid,oldNum,obj){
    	...
    	if(newNumIpt.value!=oldNum){
    		window.location.href="${app}/servlet/UpdateCartServlet?pid="+pid+"&newBuyNum="+newNumIpt.value;
    	}
    }
    function addNum2(pid,obj){
    	//var num = document.getElementById(pid).value;
    	var num = obj.parentNode.getElementsByTagName("input")[1].value;
    	var newNum = parseInt(num)+1;
    	window.location.href="${app}/servlet/UpdateCartServlet?pid="+pid+"&newBuyNum="+newNum;
    }
    ...
    function delNum(pid){
    	...
    	if(newNum>0){
    		...
    		if(window.confirm("您确认删除吗？")){
    			window.location.href="${app}/serlvet/DeleteCartServlet?pid="+pid;
    		}
    	}
    }
    </script>
  ...
  <%@include file="/_head.jsp" %>
	...
	<c:forEach items="${sessionScope.cart}" var="entry">
		<div id="prods">
			...
			<span id="prods_del"><a href="${app }/servlet/DeleteCartServlet?pid=${entry.key.id}">删除</a></span>
		</div>
	</c:forEach>
		<!-- 总计条 -->
		<div id="total">
